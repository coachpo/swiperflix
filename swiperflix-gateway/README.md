...
## Configuration
Copy `example.env` to `.env` or export manually:
...
The app keeps data in `swiperflix.db` at the gateway directory root. Deleting the file resets state; startup will recreate tables and add the `pick_count` column/index if missing.
...
All endpoints are open — no authentication required. Auth is expected at the reverse proxy layer.
